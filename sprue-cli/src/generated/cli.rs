...
impl<T: CliConfig> Cli<T> {
    ...
    pub fn get_command(cmd: CliCommand) -> ::clap::Command {
        match cmd {
            ...
            CliCommand::CancelBlobUpload => Self::cli_cancel_blob_upload(),
            CliCommand::CompleteBlobUpload => Self::cli_complete_blob_upload(),
            CliCommand::ResetBlobUpload => Self::cli_reset_blob_upload(),
            CliCommand::WriteBlobUpload => Self::cli_write_blob_upload(),
    ...
    pub fn cli_cancel_blob_upload() -> ::clap::Command {
        ::clap::Command::new("")
            .arg(
                ::clap::Arg::new("blob")
                    .long("blob")
    ...
    pub fn cli_complete_blob_upload() -> ::clap::Command {
        ::clap::Command::new("")
            .arg(
                ::clap::Arg::new("blob")
                    .long("blob")
            ...
            .about("Mark a blob as being fully uploaded and ready to be persisted.")
    }

    pub fn cli_reset_blob_upload() -> ::clap::Command {
        ::clap::Command::new("")
            .arg(
                ::clap::Arg::new("blob")
                    .long("blob")
            ...
            .about("Reset a blob to synchronously remove any data already uploaded to this blob.")
    }

    pub fn cli_write_blob_upload() -> ::clap::Command {
        ::clap::Command::new("")
            .arg(
                ::clap::Arg::new("blob")
                    .long("blob")
    ...
    pub fn cli_checkin_server() -> ::clap::Command {
        ::clap::Command::new("")
            ...
            .about("Report a check in of a server for aliveness checks")
    }

    pub fn cli_register_oidc_token_request() -> ::clap::Command {
        ::clap::Command::new("")
            .arg(
                ::clap::Arg::new("server")
                    .long("server")
        ...
        ::clap::Command::new("")
            .arg(
                ::clap::Arg::new("request")
                    .long("request")
                    .value_parser(::clap::value_parser!(types::TypedUuidForTokenRequestId))
                    .required_unless_present("json-body"),
            )
            .arg(
                ::clap::Arg::new("server")
    ...
    pub fn cli_prove_server() -> ::clap::Command {
        ::clap::Command::new("")
            .arg(
                ::clap::Arg::new("server")
                    .long("server")
            ...
            .about("Prove the identity of a server")
    }

    pub fn cli_reject_server() -> ::clap::Command {
        ::clap::Command::new("")
            .arg(
                ::clap::Arg::new("server")
                    .long("server")
    ...
    pub fn cli_get_service() -> ::clap::Command {
        ::clap::Command::new("")
            .arg(
    ...
    pub fn cli_get_service_servers() -> ::clap::Command {
        ::clap::Command::new("")
            .arg(
                ::clap::Arg::new("service")
                    .long("service")
    ...
    pub async fn execute(
        ...
        match cmd {
            ...
            CliCommand::CancelBlobUpload => self.execute_cancel_blob_upload(matches).await,
            CliCommand::CompleteBlobUpload => self.execute_complete_blob_upload(matches).await,
            CliCommand::ResetBlobUpload => self.execute_reset_blob_upload(matches).await,
            CliCommand::WriteBlobUpload => self.execute_write_blob_upload(matches).await,
    ...
    pub async fn execute_accept_server(&self, matches: &::clap::ArgMatches) -> anyhow::Result<()> {
        ...
        self.config.execute_accept_server(matches, &mut request)?;
        let result = request.send().await;
        match result {
            Ok(r) => {
                self.config.success_no_item(&r);
    ...
    pub async fn execute_register_blob(&self, matches: &::clap::ArgMatches) -> anyhow::Result<()> {
        ...
        self.config.execute_register_blob(matches, &mut request)?;
        let result = request.send().await;
        match result {
            Ok(r) => {
                self.config.success_item(&r);
    ...
    pub async fn execute_checkin_server(&self, matches: &::clap::ArgMatches) -> anyhow::Result<()> {
        ...
        self.config.execute_checkin_server(matches, &mut request)?;
        let result = request.send().await;
        match result {
            Ok(r) => {
                self.config.success_item(&r);
    ...
    pub async fn execute_prove_server(&self, matches: &::clap::ArgMatches) -> anyhow::Result<()> {
        let mut request = self.client.prove_server();
        if let Some(value) = matches.get_one::<types::TypedUuidForServerRegistrationId>("server") {
            request = request.server(value.clone());
        }
        ...
        self.config.execute_prove_server(matches, &mut request)?;
        let result = request.send().await;
        match result {
            Ok(r) => {
                self.config.success_no_item(&r);
    ...
    pub async fn execute_reject_server(&self, matches: &::clap::ArgMatches) -> anyhow::Result<()> {
        let mut request = self.client.reject_server();
        if let Some(value) = matches.get_one::<types::TypedUuidForServerRegistrationId>("server") {
            request = request.server(value.clone());
        }

        self.config.execute_reject_server(matches, &mut request)?;
        let result = request.send().await;
        match result {
            Ok(r) => {
                self.config.success_no_item(&r);
    ...
    pub async fn execute_create_service(&self, matches: &::clap::ArgMatches) -> anyhow::Result<()> {
        ...
        self.config.execute_create_service(matches, &mut request)?;
        let result = request.send().await;
        match result {
            Ok(r) => {
                self.config.success_item(&r);
    ...
    pub async fn execute_get_service(&self, matches: &::clap::ArgMatches) -> anyhow::Result<()> {
        ...
        self.config.execute_get_service(matches, &mut request)?;
        let result = request.send().await;
        match result {
            Ok(r) => {
                self.config.success_item(&r);
    ...
    pub async fn execute_register_server(
        ...
        self.config.execute_register_server(matches, &mut request)?;
        let result = request.send().await;
        match result {
            Ok(r) => {
                self.config.success_item(&r);
...
pub trait CliConfig {
    ...
    fn execute_cancel_blob_upload(
        &self,
        matches: &::clap::ArgMatches,
        request: &mut builder::CancelBlobUpload,
    ) -> anyhow::Result<()> {
        Ok(())
    }

    fn execute_complete_blob_upload(
        &self,
        matches: &::clap::ArgMatches,
        request: &mut builder::CompleteBlobUpload,
    ) -> anyhow::Result<()> {
        Ok(())
    }

    fn execute_reset_blob_upload(
        &self,
        matches: &::clap::ArgMatches,
        request: &mut builder::ResetBlobUpload,
    ) -> anyhow::Result<()> {
        Ok(())
    }

    fn execute_write_blob_upload(
        &self,
        matches: &::clap::ArgMatches,
        request: &mut builder::WriteBlobUpload,
    ) -> anyhow::Result<()> {
        Ok(())
    }
    ...
    fn execute_accept_server(
        &self,
        matches: &::clap::ArgMatches,
        request: &mut builder::AcceptServer,
    ) -> anyhow::Result<()> {
        Ok(())
    }
    ...
    fn execute_checkin_server(
        &self,
        matches: &::clap::ArgMatches,
        request: &mut builder::CheckinServer,
    ) -> anyhow::Result<()> {
        Ok(())
    }

    fn execute_register_oidc_token_request(
        &self,
        matches: &::clap::ArgMatches,
        request: &mut builder::RegisterOidcTokenRequest,
    ) -> anyhow::Result<()> {
        Ok(())
    }

    fn execute_prove_oidc_token_request(
        &self,
        matches: &::clap::ArgMatches,
        request: &mut builder::ProveOidcTokenRequest,
    ) -> anyhow::Result<()> {
        Ok(())
    }

    fn execute_prove_server(
        &self,
        matches: &::clap::ArgMatches,
        request: &mut builder::ProveServer,
    ) -> anyhow::Result<()> {
        Ok(())
    }

    fn execute_reject_server(
        &self,
        matches: &::clap::ArgMatches,
        request: &mut builder::RejectServer,
    ) -> anyhow::Result<()> {
        Ok(())
    }

    fn execute_terminate_server(
        &self,
        matches: &::clap::ArgMatches,
        request: &mut builder::TerminateServer,
    ) -> anyhow::Result<()> {
        Ok(())
    }
    ...
    fn execute_create_service(
        &self,
        matches: &::clap::ArgMatches,
        request: &mut builder::CreateService,
    ) -> anyhow::Result<()> {
        Ok(())
    }

    fn execute_get_service(
        &self,
        matches: &::clap::ArgMatches,
    ...
    fn execute_register_server(
        &self,
        matches: &::clap::ArgMatches,
        request: &mut builder::RegisterServer,
    ) -> anyhow::Result<()> {
        Ok(())
    }

    fn execute_get_service_servers(
        &self,
        matches: &::clap::ArgMatches,
        request: &mut builder::GetServiceServers,
    ) -> anyhow::Result<()> {
        Ok(())
    }
}
...
impl CliCommand {
    pub fn iter() -> impl Iterator<Item = CliCommand> {
        vec![
            ...
            CliCommand::CancelBlobUpload,
            CliCommand::CompleteBlobUpload,
            CliCommand::ResetBlobUpload,
            CliCommand::WriteBlobUpload,
