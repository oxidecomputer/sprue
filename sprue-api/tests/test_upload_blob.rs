...
async fn test_upload_blob() {
    ...
    let registration = vm.client.register_server().service(service.id.0).body_map(|body| {
        body.instance(vm.conf.uuid)
    }).send().await.unwrap().into_inner().registration;
    ...
    vm.client.prove_server().server(registration.id.0).body_map(|body| {
    ...
    user.client.accept_server().server(registration.id.0).send().await.unwrap();

    // The server now needs a token to interact with the API
    let token_request = vm.client.register_oidc_token_request().server(registration.id.0).send().await.unwrap().into_inner();
    tracing::debug!(?token_request, "Requested token");
    let token_qualifying_data = nonce_to_data(&token_request.nonce.as_ref().unwrap());
    let token_attestation = vm.rot().attest(&vm.conf(), &token_qualifying_data).await.unwrap();
    tracing::debug!("Generated token attestation");
    let token = vm.client.prove_oidc_token_request().server(registration.id.0).body_map(|body| {
        body.attestation(serde_json::to_value(token_attestation).unwrap()).request(token_request.id.0)
    }).send().await.unwrap().into_inner().token;
    ...
    let blob = vm.client.register_blob().server(registration.id.0).body_map(|body| {
        body.idempotency_key(idempotency_key).size(1024)
    })
    .send()
    .await
    .unwrap()
    ...
    // Stream has junk data into the blob
    vm.client.write_blob_upload().blob(blob.id.0).body(body).send().await.unwrap();
}
